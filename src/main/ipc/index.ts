...
import { checkBinaries, readSettings, writeSettings } from "../service/settings";
...
export function registerIpc(): void {
  ipcMain.handle(IPC.checkBinaries, () => checkBinaries());
  ...
  ipcMain.handle(IPC.loadConfig, () => readSettings());
  ipcMain.handle(IPC.saveConfig, (_event, config: AppConfig) => writeSettings(config));
